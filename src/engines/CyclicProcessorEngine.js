/**
 * CyclicProcessorEngine — CPSE v1.0
 * Containment-Based Evolution Engine
 * S(n+1) = Contain(S(n)) + Delta
 *
 * Architecture:
 *   Core Engine         — cyclic state, velocity, history, rewind, travelTo
 *   Containment Layer   — cycle counter, memory signature per state
 *   Pipeline            — ordered processors, process(), force()
 *   Integrated Analyzer — z-normalization, behaviorVector, forecast, health
 *   Event System        — on / once / off, plugins
 */
...
export class CyclicProcessorEngine {
  ...
  constructor(options = {}) {
    ...
    if (Array.isArray(options.processors)) for (const p of options.processors) this.addProcessor(p)
    if (Array.isArray(options.plugins))    for (const p of options.plugins)    this.use(p)
  }
  ...
  process(input = null, options = {}) {
    ...
    const baseContext = {
      engine: this, cycle: this.#cycle, step: this.#step,
      cycleCount: this.#cycleCount, memorySignature: this.#memorySignature,
      deltaTime, input, options,
    }
    ...
    for (const processor of this.#processors) {
      const result = processor({
        ...
        input,
        context: { ...baseContext, state: this.#normalize(previous + totalStep) },
      })
      if (!result) continue
    ...
    let appliedStep = totalStep
    let velocity    = 0

    if (totalStep !== 0) {
      velocity = Math.abs(totalStep) / deltaTime
      if (this.#maxVelocity !== Infinity && velocity > this.#maxVelocity) {
        appliedStep = Math.sign(totalStep) * this.#maxVelocity * deltaTime
        this.#emit("velocityExceeded", { requested: totalStep, clamped: appliedStep, velocity, maxVelocity: this.#maxVelocity, deltaTime, timestamp: now })
      }
    }

    const next = this.#applyContainment(previous, appliedStep)
    this.#state = next

    const entry = this.#record({
      type: "process", previous, next,
      step: appliedStep, requestedStep: totalStep,
      velocity: totalStep !== 0 ? velocity : undefined,
      cycleCount: this.#cycleCount, memorySignature: this.#memorySignature,
      input, output: outputs, cycle: this.#cycle, timestamp: now,
    })
  ...
  tick(input = null) { return this.process(input) }
  ...
  force(step) {
    ...
    const now       = this.#clock()
    const deltaTime = Math.max(now - this.#lastTimestamp, 1)
    const velocity  = Math.abs(step) / deltaTime
    let   clamped   = step

    if (this.#maxVelocity !== Infinity && velocity > this.#maxVelocity) {
      clamped = Math.sign(step) * this.#maxVelocity * deltaTime
      this.#emit("velocityExceeded", { requested: step, clamped, velocity, maxVelocity: this.#maxVelocity, deltaTime, timestamp: now })
    }

    this.#lastTimestamp = now
    const previous = this.#state
    const next     = this.#applyContainment(previous, clamped)
    this.#state    = next

    const entry = this.#record({
      type: "transition", previous, next, step: clamped, velocity,
      cycleCount: this.#cycleCount, memorySignature: this.#memorySignature,
      cycle: this.#cycle, timestamp: now,
    })
  ...
  signedDistance(from, to, options = {}) {
    ...
    if (Math.abs(backward) === Math.abs(forward)) return prefer === "backward" ? backward : forward
  ...
  rewind(steps = 1) {
    ...
    const payload = { type: "rewind", previous: entry.next, next: entry.previous, steps, cycle: this.#cycle, timestamp: this.#lastTimestamp }
  ...
  travelTo(index) {
    ...
    const payload = { type: "travel", previous: entry.previous, next: entry.next, index, cycle: this.#cycle, timestamp: this.#lastTimestamp }
  ...
  snapshot() {
    const snap = {
      state: this.#state, step: this.#step, cycle: this.#cycle,
      maxVelocity: this.#maxVelocity, cycleCount: this.#cycleCount,
      memorySignature: this.#memorySignature,
      history: this.getHistory(), timestamp: this.#clock(),
    }
  ...
  restore(snapshot) {
    ...
    if (Number.isFinite(snapshot.maxVelocity) && snapshot.maxVelocity > 0) this.#maxVelocity = snapshot.maxVelocity
    if (this.#history.length > this.#maxHistory) this.#history = this.#history.slice(-this.#maxHistory)
    this.#lastTimestamp = this.#clock()

    this.#emit("restore", { previous, next: this.#state, cycleCount: this.#cycleCount, memorySignature: this.#memorySignature, historySize: this.#history.length, timestamp: this.#lastTimestamp })
    return this
  }
  ...
  reset(state = 0) {
    ...
    this.#memorySignature = 0
    this.#lastTimestamp   = this.#clock()
    this.#emit("reset", { previous, next: this.#state, cycle: this.#cycle, timestamp: this.#lastTimestamp })
    return this
  }
  ...
  analyze(value) {
    ...
    if (history.length > 1) {
      ...
      az.adaptiveThreshold = Math.max(az.baseThreshold, Math.min(raw, ceiling))
    }
    ...
    const confidence       = similarity !== null ? Math.min(99, Math.round(50 + similarity * 49)) : Math.min(99, Math.round(40 + (history.length / az.historyWindow) * 50))
    ...
    const report = {
      status, severity: az.lastSeverity, trend, eta, confidence, confidenceSource,
      ...
      behaviorVector, health, forecast, containment,
      explain: {
  ...
  learnPattern(values) {
    ...
    for (let i = 1; i < values.length; i++) {
      if (!Number.isFinite(values[i]) || !Number.isFinite(values[i - 1])) throw new Error("CPE_ANALYZER_INVALID_LEARN_VALUE_ENTRY")
  ...
  once(event, listener) {
    if (!this.#listeners[event])        throw new Error("CPE_INVALID_EVENT")
    if (typeof listener !== "function") throw new Error("CPE_INVALID_LISTENER")
    const wrapper = (payload, engine) => { listener(payload, engine); this.off(event, wrapper) }
  ...
  use(plugin) {
    if (typeof plugin !== "function" && (!plugin || typeof plugin.install !== "function")) throw new Error("CPE_INVALID_PLUGIN")
  ...
  unuse(plugin) { this.#plugins.delete(plugin); return this }
  ...
  #applyContainment(previous, step) {
    ...
    const next = this.#normalize(previous + step)
    if (this.#didWrap(previous, next, step)) {
      this.#cycleCount += 1
      this.#emit("cycleComplete", { cycleCount: this.#cycleCount, memorySignature: this.#memorySignature, previous, next, timestamp: this.#clock() })
    }
    const phi = 1.6180339887
    this.#memorySignature = Math.round(((this.#memorySignature * phi + next) % this.#cycle) * 1000) / 1000
    return next
  }
  ...
  #record(payload) {
    const entry = Object.freeze({ ...payload, timestamp: Number.isFinite(payload.timestamp) ? payload.timestamp : this.#clock() })
  ...
  #sanitizeEntry(entry) {
    if (!entry || !Number.isFinite(entry.previous) || !Number.isFinite(entry.next)) throw new Error("CPE_INVALID_HISTORY_ENTRY")
  ...
  #slidingWindowSimilarity(currentSteps, learnedZScores) {
    ...
    let sumDiff = 0
    for (let i = 0; i < windowSize; i++) sumDiff += Math.pow(cNorm[i] - lWindow[i], 2)
  ...
  #linearSlope(buffer) {
    ...
    let num = 0, den = 0
    for (let i = 0; i < n; i++) { num += (i - xm) * (buffer[i] - ym); den += Math.pow(i - xm, 2) }
  ...
  #computeBehaviorVector(stdDev, deviation, similarity) {
    ...
    const behaviorScore  = Math.round((stabilityScore * (rem * 0.5) + (1 - deviationScore) * (rem * 0.5) + patternScore * patternWeight) * 100)
    return {
  ...
  #computeForecast(currentScore) {
    ...
    const intervalsTo = (target) => { const gap = currentScore - target; return gap <= 0 ? null : Math.round(gap / degradationPerInterval) }
    return {
