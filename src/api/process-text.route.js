...
function feed(sid, text) {
  ...
  return {
    ...
    celfResult: {
      ...
      field,
      metrics,
      perturbation: snapshot.perturbation ?? {},
      attractors: snapshot.attractors ?? []
    }
  }
}
...
function engine_cosine(a, b) {
  ...
  return (na > 0 && nb > 0)
    ? Math.max(0, Math.min(1, dot / (Math.sqrt(na) * Math.sqrt(nb))))
    : 0
}
...
function detectCodeBlocks(text) {
  ...
  if (blocks.length === 0) {
    ...
    if (codeSignals.filter(p => p.test(text)).length >= 2 && text.length > 50) {
      blocks.push(text)
    }
  }
...
function storeCodeContext(sid, rawArr, engine, tValue) {
  ...
  if (contexts.length > 10) {
    contexts.splice(0, contexts.length - 10)
  }
...
function retrieveRelevantCode(questionVector, questionText, sid, currentMsgIndex) {
  ...
  for (const ctx of contexts) {
    const codeSim = questionVector && ctx.codeVector
      ? engine_cosine(questionVector, ctx.codeVector)
      : 0

    const summarySim = questionVector && ctx.summaryVector
      ? engine_cosine(questionVector, ctx.summaryVector)
      : 0
    ...
    const finalScore =
      (codeSim * 0.55 + summarySim * 0.30 + Math.min(0.30, symbolBoost) * 0.15) * 0.85 +
      freshness * 0.15

    if (finalScore > bestScore && finalScore > 0.40) {
...
function buildAnchorContext(sid) {
  ...
  const top = anchors
    .slice(0, 3)
    .map(a => `${a.concept}(${Math.round(a.weight * 100)}%)`)
    .join(', ')
...
async function generateSessionSummary(sid, history, engine) {
  ...
  const decisions = recent
    .filter(h => h.role === 'user' && /قررنا|decided|we.ll use/i.test(h.content))
    .map(h => h.content.slice(0, 80))
    .slice(0, 3)

  return {
    text: `${domain}: ${symbols || 'general'} — ${mainTopic}`.slice(0, 200),
    decisions,
...
function buildHistoryLayer(history, continuity, sid, needsRawCode = false, currentDomain = 'general') {
  ...
  if (continuity >= 0.40) {
    const msgs = clean.slice(-4).map(h => ({
      role: h.role,
      content: h.role === 'assistant'
        ? compressAssistantMessage(h.content)
        : needsRawCode
...
function buildFieldSignals(sid, celfResult, cleanedText, codeBlocks, continuity, anchors = [], hasStoredCode = false) {
  ...
  if (primaryIntent && ANCHOR_TO_INTENT[primaryIntent]) {
    add(ANCHOR_TO_INTENT[primaryIntent], 0.90)
  }
  ...
  if (/مشروع|project|continuation/i.test(cleanedText) && continuity > 0.50) {
    add('#project_continuation', 0.80)
  }
  ...
  if (continuity > 0.20 && (state.driftCount ?? 0) === 0) {
    add('#followup', 0.60)
  }
...
router.post('/process-text', async (req, res) => {
  ...
  try {
    ...
    if (
      !rawCodeStore.has(sid) &&
      recoveredCode &&
      typeof recoveredCode === 'string' &&
      recoveredCode.length > 30
    ) {
    ...
    if (!resumeBootstrapped.has(sid) && (sessionSummary?.text || recoveredCode)) {
      ...
      if (resumeText.trim()) {
        try {
          engine.process(resumeText, { sourceWeight: 0.65 })
        } catch {}
      }
    ...
    const styleHint = activeStyle && styleMap[activeStyle]
      ? styleMap[activeStyle]
      : null

    const systemParts = [directives, styleHint].filter(Boolean)

    if (activeSummary?.text) {
      systemParts.unshift(`[session] ${activeSummary.text}`)
    }
    ...
    const historyMessages = buildHistoryLayer(
      filteredHistory,
      continuity,
      sid,
      !!storedRaw,
    ...
    const recCode =
      typeof recoveredCode === 'string' && recoveredCode.length > 30
    ...
    if (reply && !hasImage && tValue > 1 && wordCount > 2 && questionVector?.length) {
      try {
        observerBox = observe({
          ...
          replyText: reply,
          noiseRemoved: false,
          lang: userIsArabic ? 'ar' : 'en'
        })

        if (observerBox) {
          storeCapsule(sid, observerBox, cleanedText.slice(0, 80), tValue)
          updateAnchors(sid, cleanedText.slice(0, 80), 0.5)
        }
      } catch {}
    }
    ...
    if (msgCountAfter % SUMMARY_INTERVAL === 0) {
      try {
        ...
        if (newSummary) {
          sessionSummaryStore.set(sid, newSummary)
        }
      } catch {}
    }
...
router.get('/session/:id', (req, res) => {
  ...
  return res.json({
    ...
    summary: sessions.get(req.params.id).getSummary?.() ?? {}
  })
})
...
router.delete('/session/:id', (req, res) => {
  ...
  semanticTextMaps.delete(id)
  styleStore.delete(id)
  processingLock.delete(id)
  _semanticState.delete(id)
  _fieldHistory.delete(id)
  _entityTracker.delete(id)
  rawCodeStore.delete(id)
  codeSessionStore.delete(id)
  resumeBootstrapped.delete(id)
  capsuleMemory.delete(id)
