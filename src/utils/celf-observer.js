// ═══════════════════════════════════════════════════════════════
//  celf-observer.js — v5.0
//
//  المواصفة النهائية:
//  ┌─────────────────────┬────────────────────┬──────────────┐
//  │ القيمة              │ المصدر             │ الموثوقية    │
//  ├─────────────────────┼────────────────────┼──────────────┤
//  │ coverage            │ CONCEPT_ALIASES    │ ✅ موثوق     │
//  │ memoryContinuity    │ capsule sim        │ ⚠️ جزئي      │
//  │ relevance           │ hash vectors       │ diagnostics  │
//  └─────────────────────┴────────────────────┴──────────────┘
//
//  القواعد:
//  - relevance لا تظهر للمستخدم أبداً
//  - coverage تظهر فقط إذا ratio != null
//  - hints تظهر فقط إذا missing > 5 أحرف
//  - max 3 ملاحظات، max 2 hints
// ═══════════════════════════════════════════════════════════════
...
const STYLE_WORDS = new Set([
  // عربي — أسلوب وطلب
  'بطريقة','طريقة','بشكل','شكل','دقيقة','دقيق','علمية','علمي','مفصلة','مفصل','بسيطة','بسيط',
  'واضحة','واضح','شاملة','شامل','سريعة','سريع','موجزة','موجز','عملية','عملي','نظرية','نظري',
  ...
  'genau','einfach','detailliert','wissenschaftlich','klar','kurz','vollständig','praktisch',
  'theoretisch','präzise','korrekt','ausführlich','genauer','detaillierte','verständliche','verständlich',
  'kannst','erklären','erkläre','bitte','mehr','zeige','schreibe','mache','nenne','erklar','erklärung',
  'biite','sag','beschreibe','definiere','zeig','nenn','was','ist','wie','warum','welche','welches',
  'welcher','wann','wer','gibt','können','könntest','beigetragen','entwicklung','neuer','neue','neues',
  'konzepte','konzept','zusammenhang','unterschied','bedeutung','beispiel','ergebnis','antwort','antwot',
...
const CONCEPT_ALIASES = [
  ...
  { id: 'quantum_computing', labels: { ar: 'الحوسبة الكمومية',  de: 'Quantum Computing',      en: 'quantum computing'  }, aliases: ['quantum computing','quantencomputer','quantencomputing','qubits','qubit','حاسوب كمومي','الحوسبة الكمومية'] },
  ...
  { id: 'bell_theorem',      labels: { ar: 'مبرهنة بيل',        de: 'Bell-Theorem',           en: 'Bell theorem'       }, aliases: ['bell','bell theorem','bells theorem','bell-theorem','bell ungleichung','مبرهنة بيل','بيل'] },
...
function extractFallbackConcepts(text, lang) {
  ...
  return [...new Set(tokenize(text))]
    ...
    .filter(t => !known.some(k =>
      k.aliases.some(a => includesAlias(t, a) || includesAlias(a, t))))
    .slice(0, 4)
    .map(t => ({ id: `term:${t}`, label: t, aliases: [t] }))
}

function extractConcepts(text, lang) {
  // أسئلة قصيرة ≤ 3 كلمات: لا نستخرج fallback — غير موثوق
  const wordCount = tokenize(text).length
  const known     = extractKnownConcepts(text, lang)
  if (wordCount <= 3) return known
...
function measureConceptCoverage(questionText, replyText, lang) {
  ...
  return {
    ratio:    results.filter(r => r.covered).length / results.length,
    covered:  results.filter(r =>  r.covered).map(r => r.label).slice(0, 5),
    missing:  results.filter(r => !r.covered).map(r => r.label).slice(0, 3),
    concepts: results
  }
}

// memoryContinuity — جزئي ⚠️
...
// relevance — في diagnostics فقط ❌ لا تظهر للمستخدم
...
// confidence يعتمد على coverage وحده (relevance محذوف)
function confidenceLabel(coverageRatio) {
...
// ── بناء الملاحظات — coverage + continuity فقط ───────────────────
function buildObservations(coverage, memoryContinuity, lang) {
  ...
  if (memoryContinuity !== null) {
    if (memoryContinuity >= 0.65)
      ...
      lines.push(L.memNew)
    // 0.25 < x < 0.65 → لا ملاحظة (محايد)
  }

  return lines.slice(0, 3)  // max 3
}

// ── Hints — فقط مصطلحات حقيقية > 5 أحرف ─────────────────────────
function buildNextQuestionHints(missing, lang) {
  if (!missing?.length) return []
  const L = OBS[lang] ?? OBS.en
  return missing
    .filter(t => t.length > 5)   // يتجنب الكلمات التافهة
    .slice(0, 2)                 // max 2
    .map(t => L.hint(t))
}
...
export function observe({
  ...
  return {
    // ── للمستخدم: coverage + continuity فقط ──────────────────
    observations: buildObservations(coverage, memoryContinuity, obsLang),

    // ── للنظام: كل القيم بما فيها relevance ──────────────────
    diagnostics: {
      confidence:       confidenceLabel(coverage?.ratio ?? null),
      relevance:        relevanceLabel(relevance),   // للنظام فقط
    ...
    // ── hints: مصطلحات حقيقية > 5 أحرف فقط ──────────────────
